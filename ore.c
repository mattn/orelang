...
typedef ore_value (*ore_func_t)(ore_context*, int, ore_value*);
...
ore_value
ore_define_cfunc(ore_context* ore, const char* name, ore_func_t f) {
  int r = 0;
  khint_t k = kh_put(ident, ore->env, name, &r);
  ore_value v = { ORE_TYPE_FNC };
  v.v.f = f;
...
ore_call(ore_context* ore, mpc_ast_t *t) {
  ...
  ore_value f = kh_value(ore->env, k);
  ...
  ore_value v = ((ore_func_t)f.v.f) (ore, num_in, args);
  free(args);
  return v;
}
